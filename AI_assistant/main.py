...
subprocesses=[]
Functions = ["open","close","system","play","content","gooogle search","youtube search"]
...
def MainExecution():
    ...
    if G and R or R:
        SetAssistant("searching....")
        Answer=RealtimeSearchEngine(QueryModifier(Merged_query))
        ShowTextToScreen(f"{Assistantname}:{Answer}")
        SetAssistant("Asnwering...")
        TextToSpeech(Answer)
        return True
    else:
        for Queries in Decision:
...
def FirstThread():
    while True:
        CurrentStatus = GetMicrophone()
        if CurrentStatus=="True":
            MainExecution()
        else:
            AIStatus= GetAssistant()
            if "Available..." in AIStatus:
                sleep(0.1)
            else:
                SetAssistant("Available...")
